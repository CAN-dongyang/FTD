...
public class ProfessorHirePopupUI : MonoBehaviour
{
    ...
    public void Open(ProfessorData data)
    {
        _data = data;
    ...
    private void UpdateUI()
    {
        ...
        bool isHired = ProfessorDataManager.Instance != null && ProfessorDataManager.Instance.HiredProfessors.Any(p => p.Id == _data.Id);
    ...
    private void OnHireClicked()
    {
        ...
        {
            bool success = ProfessorDataManager.Instance.HireProfessor(_data.Id);
            if (success)
            {
                SetHiredState(true);
                if (ProfessorListUI.Instance != null)
                {
                    ProfessorListUI.Instance.UpdateList();
                }
                // 고용된 교수 목록 UI 갱신
                if (HiredProfessorListUI.Instance != null)
                {
                    HiredProfessorListUI.Instance.UpdateList();
                }
    ...
    private void OnFireClicked()
    {
        if (_data != null && ProfessorDataManager.Instance != null)
        {
            bool success = ProfessorDataManager.Instance.FireProfessor(_data.Id);
            if (success)
            {
                SetHiredState(false);
                if (ProfessorListUI.Instance != null)
